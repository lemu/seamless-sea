...
/**
 * Manually link a Clerk user to an existing Convex user by email
 * This is useful for migrating existing users to Clerk
 */
export const linkClerkUser = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (user.clerkUserId && user.clerkUserId !== args.clerkUserId) {
      throw new Error(
        `User already linked to different Clerk ID: ${user.clerkUserId}`
      );
    }
    ...
    console.log(`Linked user ${args.email} to Clerk ID ${args.clerkUserId}`);
